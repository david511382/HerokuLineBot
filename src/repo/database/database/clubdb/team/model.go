...
type Model struct {
	...
	Name                string     `gorm:"column:name;type:varchar(64);not null;uniqueIndex:uniq_name_ownerid,priority:1"`
	CreateDate          time.Time  `gorm:"column:create_date;type:date;not null;comment:欄位"`
	DeleteAt            *time.Time `gorm:"column:delete_at;index"`
	OwnerMemberID       uint       `gorm:"column:owner_member_id;type:int unsigned;not null;uniqueIndex:uniq_name_ownerid,priority:2"`
	NotifyLineRommID    *string    `gorm:"column:notify_line_room_id;type:varchar(64)"`
	ActivityDescription *string    `gorm:"column:activity_description;type:varchar(64)"`
