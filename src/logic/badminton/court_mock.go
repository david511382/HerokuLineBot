...
func (m *MockIBadmintonCourtLogic) GetCourts(fromDate, toDate *time.Time, dates []*time.Time, teamID, placeID *uint) (map[uint]map[uint][]*DateCourt, error.IError) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetCourts", fromDate, toDate, dates, teamID, placeID)
	ret0, _ := ret[0].(map[uint]map[uint][]*DateCourt)
